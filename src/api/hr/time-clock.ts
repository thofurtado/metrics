...
export async function getTimeClockStatus(pin: string) {
  const response = await api.get<TimeClockStatusResponse>(
    '/hr/time-clock/status',
    {
      params: { pin },
    },
  )
...
export async function registerTimeClock(pin: string, action: string) {
  const response = await api.post<RegisterTimeClockResponse>(
    '/hr/time-clock/register',
    {
      pin,
      action,
    },
...
export async function updateTimeClock(
  id: string,
  data: Partial<TimeClock> & {
    isExtraDay?: boolean
