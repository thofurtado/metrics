...
import { auditSession, getSessionDetails } from '@/api/cashier/cashier'
    ...
    <div className="space-y-6 p-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          <Button
            variant="outline"
            onClick={() => navigate('/admin/cashier/sessions')}
          >
            Voltar
          </Button>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            <p>
              <strong>Status:</strong> <Badge>{session.status}</Badge>
            </p>
            <p>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <p>
              <strong>Total Calculado (Sistema):</strong>{' '}
              {formatCurrency(session.calculated_total)}
            </p>
            <p>
              <strong>Total Informado (Operador):</strong>{' '}
              {session.reported_total !== undefined
                ? formatCurrency(session.reported_total)
                : '-'}
            </p>
            {session.difference !== undefined && (
              <p
                className={`font-bold ${session.difference < 0 ? 'text-red-500' : session.difference > 0 ? 'text-green-500' : ''}`}
              >
                <strong>Diferença:</strong> {formatCurrency(session.difference)}
              </p>
            )}
            <p>
              <strong>Observações:</strong> {session.notes || 'Nenhuma'}
            </p>
          </CardContent>
        </Card>
      </div>
