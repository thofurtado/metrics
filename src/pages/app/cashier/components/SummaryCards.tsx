...
export function SummaryCards({
  ...
  const bancosComValor = bancosDinamicos.filter(
    (b) => safeGet(resumo, `${b}.total`) > 0,
  )
  const temConsumoInterno = safeGet(resumo, 'CASA.total') > 0

  // Totais consolidados de maquininhas
  const totalMaquininhas = bancosComValor.reduce(
    (acc, b) => acc + safeGet(resumo, `${b}.total`),
    0,
  )
  const totalJuros = bancosComValor.reduce(
    (acc, b) => acc + safeGet(resumo, `${b}.juros`),
    0,
  )
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-4">
        ...
        {(() => {
          ...
          identificadoresCasa.forEach((forma) => {
            const isAPrazo = padraoAPrazo.some((p) =>
              normalizeStr(forma).includes(p),
            )
          ...
          return (
            <div className="relative overflow-hidden rounded-2xl border border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50/60 p-4 shadow-sm dark:border-blue-900/40 dark:from-blue-950/30 dark:to-indigo-950/20">
              <Banknote
                size={34}
                className="absolute -right-1 -top-1 rotate-12 text-blue-500 opacity-10"
              />
              ...
              <div className="mt-3 space-y-1 border-t border-blue-200/60 pt-2 dark:border-blue-800/40">
                ...
                {totalPorForma('Débito') +
                  totalPorForma('Crédito') +
                  totalPorForma('Voucher') >
                  0 && (
                  <div className="flex items-center justify-between text-[10px]">
                    <span className="flex items-center gap-1 font-medium text-slate-500 dark:text-slate-400">
                      <Plus size={8} /> Cartões
        ...
        <div className="group relative overflow-hidden rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm dark:border-emerald-950/40 dark:bg-slate-950">
          <Banknote
            size={34}
            className="opacity-8 absolute -right-1 -top-1 rotate-12 text-emerald-500"
          />
          ...
          <div className="space-y-1.5 text-[11px]">
            <div className="flex items-center justify-between text-slate-500">
              <span className="text-[10px] italic underline decoration-emerald-200">
                Abertura
              </span>
              {isEditing ? (
                ...
                >
                  ...
                  <Edit2
                    size={10}
                    className="text-emerald-500 opacity-0 group-hover/btn:opacity-100"
                  />
                </div>
              )}
            </div>
            ...
            {saidasDinheiro > 0 && (
              <div className="flex justify-between">
                <span className="flex items-center gap-1 text-red-500">
                  <ArrowDownRight size={10} /> Sangrias
                </span>
            ...
            {totalPorForma('Débito') > 0 && (
              <div className="flex items-center justify-between text-slate-500">
                ...
                <span className="font-mono font-bold text-blue-700">
                  R$ {totalPorForma('Débito').toFixed(2)}
                </span>
              </div>
            )}
            {totalPorForma('Crédito') > 0 && (
              <div className="flex items-center justify-between text-slate-500">
                ...
                <span className="font-mono font-bold text-indigo-700">
                  R$ {totalPorForma('Crédito').toFixed(2)}
                </span>
              </div>
            )}
            {totalPorForma('Voucher') > 0 && (
              <div className="flex items-center justify-between text-slate-500">
                ...
                <span className="font-mono font-bold text-purple-700">
                  R$ {totalPorForma('Voucher').toFixed(2)}
                </span>
              </div>
            )}
            {totalMaquininhas > 0 && (
        ...
        {temConsumoInterno ? (
          <div className="rounded-2xl border border-amber-100 bg-amber-50/30 p-4 shadow-sm dark:border-amber-950/40 dark:bg-amber-950/10">
            <div className="mb-3 flex items-center gap-1.5">
              ...
              <h2 className="text-[10px] font-black uppercase italic tracking-wider text-amber-700 dark:text-amber-400">
                Consumo Interno
              </h2>
            </div>
            <div className="space-y-1.5 text-[11px]">
              ...
              <div className="mt-2 flex items-center justify-between border-t border-amber-200/60 pt-2 dark:border-amber-900/40">
                <span className="text-[9px] font-black uppercase italic text-amber-700">
                  Total A Prazo
                </span>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50/30 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/30">
            <div className="mb-3 flex items-center gap-1.5">
              <span className="text-sm">🔄</span>
              <h2 className="text-[10px] font-black uppercase italic tracking-wider text-slate-400">
                Consumo Interno
              </h2>
            </div>
            ...
            >
              ...
              <div className="space-y-1.5 text-[11px]">
                {safeGet(resumo, `${banco}.PIX`) > 0 && (
                  <div className="flex justify-between text-slate-500">
                    <span className="flex items-center gap-1">
                      <Smartphone size={10} className="text-teal-500" /> PIX
                    </span>
                ...
                {safeGet(resumo, `${banco}.Débito`) > 0 && (
                  <div className="flex justify-between text-slate-500">
                    <span className="flex items-center gap-1">
                      <CreditCard size={10} className="text-blue-500" /> Débito
                    </span>
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-200">
                ...
                {safeGet(resumo, `${banco}.Voucher`) > 0 && (
                  <div className="flex justify-between text-slate-500">
                    <span className="flex items-center gap-1">
                      <Ticket size={10} className="text-purple-500" /> Voucher
                    </span>
                    <span className="font-mono font-bold text-slate-800 dark:text-slate-200">
                ...
                {safeGet(resumo, `${banco}.caixinha`) > 0 && (
                  <div className="flex justify-between text-pink-500">
                    <span className="flex items-center gap-1">
                      <Heart size={10} fill="currentColor" /> Gorjeta
                    </span>
                    <span className="font-mono font-bold">
                      {safeGet(resumo, `${banco}.caixinha`).toFixed(2)}
                    </span>
                  </div>
                )}
                {safeGet(resumo, `${banco}.recebimentoCaixa`) > 0 && (
                  <div className="flex justify-between rounded bg-emerald-50 px-1 py-0.5 text-emerald-600 dark:bg-emerald-950/20">
                    <span className="flex items-center gap-1">
                      <Banknote size={10} /> Depósito Dinheiro
                    </span>
                    <span className="font-mono font-bold">
                      + R${' '}
                      {safeGet(resumo, `${banco}.recebimentoCaixa`).toFixed(2)}
                    </span>
                  </div>
                )}
                <div className="mt-2 flex flex-col gap-1 border-t border-slate-100 pt-2 dark:border-slate-900">
