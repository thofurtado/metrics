...
export function CaixinhasTable({ lancamentos = [] }: { lancamentos?: any[] }) {
  ...
  return (
    <div className="mt-8 overflow-hidden rounded-[1.5rem] border border-pink-100 bg-white shadow-sm md:rounded-3xl">
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-zinc-100">
            {caixinhas.map((l, i) => (
              <tr
                key={l.id || i}
                className="text-xs font-bold text-zinc-600 transition-colors hover:bg-pink-50/30"
              >
