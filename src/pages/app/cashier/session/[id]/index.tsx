...
export function CashierSessionDetails() {
  ...
  const { mutateAsync: editBalance } = useMutation({
    ...
    onError: (err: any) => {
      toast.error(
        err?.response?.data?.message || 'Erro ao atualizar saldo inicial.',
      )
    },
  })
  ...
  const { mutateAsync: submitForReview } = useMutation({
    ...
    onError: (err: any) => {
      toast.error(
        err?.response?.data?.message || 'Erro ao enviar para conferência.',
      )
    },
  })
  ...
  if (!data || !data.session) {
    return (
      <div className="p-8 text-center text-zinc-500">Caixa não encontrado.</div>
    )
  }
  ...
  const mapStatus = (apiStatus: string) => {
    ...
    if (s === 'AUDITED' || s === 'CHECKED' || s === 'CONFERIDO')
      return 'CHECKED'
  ...
  const mappedLancamentos = (entries || [])
    ...
    .map((e: any) => {
      ...
      const consumidorNome =
        ...
        e.employee?.name ||
        (FORMAS_CASA.some(
          (f) => normalizeStr(f) === normalizeStr(e.payment_method),
        ) &&
        cleanIdentification &&
    ...
    for (const entry of entriesList || []) {
      ...
      if (entry.is_withdrawal) {
        ...
        if (entry.type === 'SANGRIA_DESTINO') {
          ...
          res[destBank].recebimentoCaixa =
            (res[destBank].recebimentoCaixa || 0) + amount
        }
        continue
      }
      ...
      // Se for Consumo Interno / Conta Casa / Identificador
        ...
        bank === 'CONTA DA CASA' ||
        padraoCasa.some((p) => method.toLowerCase().includes(p))
      ) {
      ...
      // Se for Maquininha / Banco digital (SAFRA, STONE, NUBANK, PAGBANK, CIELO, IFOOD, etc.)
        ...
        if (method.toUpperCase() === 'PIX') {
          ...
          method.toLowerCase().includes('débito') ||
          method.toLowerCase().includes('debito')
        ) {
          formaKey = 'Débito'
          categoryNorm = 'DEBITO'
        } else if (
          method.toLowerCase().includes('crédito') ||
          method.toLowerCase().includes('credito')
        ) {
          formaKey = 'Crédito'
          categoryNorm = 'CREDITO'
        } else if (method.toLowerCase().includes('voucher')) {
          formaKey = 'Voucher'
          categoryNorm = 'VOUCHER'
        }
        ...
        if (categoryNorm && machines && machines.length > 0) {
          const machine = machines.find(
            (m: any) => m.name.trim().toUpperCase() === bank.toUpperCase(),
          )
          if (machine && machine.rates && machine.rates.length > 0) {
            ...
            const rate = machine.rates.find((r: any) =>
              normalizeStr(r.payment_category).includes(target),
            )
  ...
  const resumoLote =
    summary && Object.keys(summary).length > 0 && summary.GERAL
      ? summary
      : computeResumo(session, entries, dbMachines || [])
  ...
  const handleAdicionarLancamento = async (dados: any) => {
    ...
    try {
      await addEntry({
        ...
        identification:
          dados.identificacao || (dados.mesa ? `Mesa ${dados.mesa}` : ''),
      ...
      alert(
        err?.response?.data?.message ||
          'Erro ao adicionar lançamento no caixa.',
      )
    }
  }
    ...
    <div className="relative">
      <DetalheLote
        ...
        onConferirECaixaConferido={
          isAdmin ? handleConferirECaixaConferido : undefined
        }
        onEnviarParaConferencia={
          session.status === 'OPEN'
            ? isAdmin
              ? handleEnviarParaConferenciaComoAdmin
              : handleEnviarParaConferenciaComoOperador
            : undefined
        }
        isAdmin={isAdmin}
        onOrderCompleted={() => {
