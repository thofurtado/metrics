...
export function ReadjustTransactionGroupDialog({
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onError: (err: any) => {
      toast.error(
        err.response?.data?.message || 'Erro ao reajustar parcelamento.',
      )
    },
  })
    ...
    >
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Escolha entre renegociar as parcelas pendentes ou corrigir o valor
        ...
        <Tabs
          value={mode}
          onValueChange={(v) => setMode(v as 'renegotiate' | 'fix')}
          className="mt-4"
        >
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
            <TabsContent value="renegotiate" className="space-y-4">
              <div className="rounded-md bg-muted p-3 text-sm">
                <p>
                  <strong>Pago até agora:</strong> R${' '}
                  {alreadyPaidAmount.toFixed(2)} ({alreadyPaidCount} parcelas)
                </p>
                <p>
                  <strong>Saldo Pendente Atual:</strong> R${' '}
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  placeholder={`Ex: ${pendingSum.toFixed(2)}`}
                />
                <span className="text-xs text-muted-foreground">
                  Deixe em branco para manter o saldo pendente atual (sem
                  juros).
                </span>
                {errors.totalAmount && (
                  <span className="text-xs text-red-500">
            ...
            <TabsContent value="fix" className="space-y-4">
              <div className="rounded-md border-l-4 border-yellow-500 bg-muted p-3 text-sm">
                <p>
                  Use esta opção apenas se lançou o contrato errado. O sistema
                  calculará o que falta baseado no novo total e no que já foi
                  pago.
                </p>
                <p className="mt-2 font-medium">
                  Já pago: R$ {alreadyPaidAmount.toFixed(2)} em{' '}
                  {alreadyPaidCount} vezes.
                </p>
              </div>
