...
import { ResponsiveDialogDescription } from '@/components/ui/responsive-dialog'
import {
  Table,
  TableBody,
  TableCell,
...
export function TransactionGroupDetailsDialog({
  groupId,
  open,
  onOpenChange,
}: TransactionGroupDetailsDialogProps) {
  ...
  const { mutateAsync: deleteFn, isPending: isDeleting } = useMutation({
    ...
    onError: () => {
      toast.error(
        'Erro ao excluir o parcelamento. Verifique se existem transações já baixadas que precisam ser revertidas primeiro.',
      )
    },
  })
  ...
  async function handleTerminate(lastKeptId: string) {
    ...
    if (
      !confirm(
        'Tem certeza que deseja encerrar o contrato MANTENDO esta parcela e removendo as futuras?',
      )
    )
      return
  ...
  async function handleDeleteGroup() {
    ...
    if (
      !confirm(
        'Tem certeza que deseja EXCLUIR TODOS OS VÍNCULOS e TODAS AS TRANSAÇÕES deste parcelamento? Esta ação não pode ser desfeita.',
      )
    )
      return
  ...
  const alreadyPaidCount =
    groupDetails?.transactions.filter((t) => t.confirmed).length || 0
  const alreadyPaidAmount =
    groupDetails?.transactions
      .filter((t) => t.confirmed)
      .reduce((acc, t) => acc + (t.totalValue ?? t.amount), 0) || 0
  const pendingSum =
    groupDetails?.transactions
      .filter((t) => !t.confirmed)
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-h-[80vh] max-w-3xl overflow-y-auto">
          <DialogHeader>
            ...
            <ResponsiveDialogDescription className="mt-1.5 flex flex-col gap-1 text-sm text-slate-600 dark:text-slate-400">
              <span>Gerencie ou encerre as parcelas deste contrato.</span>
              <span className="text-[11px] font-semibold uppercase tracking-wider text-slate-500/80">
                Clique no status para alterar rapidamente.
              </span>
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              <div className="mb-2 flex flex-col justify-between gap-4 rounded bg-muted p-2 text-sm sm:flex-row sm:items-center">
                <div className="flex gap-4">
                  <div>
                    <strong>Total Contrato:</strong> R${' '}
                    {groupDetails.totalAmount.toFixed(2)}
                  </div>
                  <div>
                    <strong>Ocorrências:</strong>{' '}
                    {groupDetails.installmentsCount}
                  </div>
                </div>
                ...
                <Table>
                  ...
                  <TableBody>
                    {groupDetails.transactions.map((tx, index) => {
                      const isLast =
                        index === groupDetails.transactions.length - 1

                      return (
                        <TableRow key={tx.id}>
                          <TableCell className="whitespace-nowrap">
                            {dayjs(tx.data_vencimento).format('DD/MM/YYYY')}
                          </TableCell>
                          ...
                          <TableCell>
                            <span
                              ...
                              className={cn(
                                'cursor-pointer select-none whitespace-nowrap rounded border border-transparent px-2 py-1 transition-colors hover:border-border',
                                tx.confirmed
                                  ? 'font-bold text-green-600 hover:bg-green-50'
                                  : 'font-medium text-yellow-600 hover:bg-yellow-50',
                              )}
                              title={
                                tx.confirmed
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {paymentAction?.type === 'pay' ? (
                <>
                  Deseja marcar esta parcela de{' '}
                  <strong className="text-foreground">
                    R$ {paymentAction?.amount.toFixed(2)}
                  </strong>{' '}
                  como <strong>PAGA</strong>?<br />A data do pagamento será
                  definida para <strong>hoje</strong>.
                </>
              ) : (
                <>
                  Deseja marcar esta parcela como <strong>PENDENTE</strong>?
                  <br />A data do pagamento e conta serão removidos.
                </>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
