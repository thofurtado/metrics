...
const employeeFormSchema = z.object({
  ...
  transportAllowance: z.preprocess(
    (val) =>
      val === '' || val === undefined || val === null ? 0 : Number(val),
    ...
    if (open) {
      form.reset({
        ...
        admissionDate: employee?.admissionDate
          ? new Date(employee.admissionDate).toISOString().split('T')[0]
  ...
  const { mutateAsync: saveEmployee, isPending } = useMutation({
    mutationFn: async (data: EmployeeFormValues) => {
      ...
      if (data.registrationType === 'DISMISSED') {
        ...
        submissionData.registrationType =
      ...
      if (submissionData.admissionDate) {
        ...
        submissionData.admissionDate = new Date(
          submissionData.admissionDate,
        ).toISOString()
      }
      ...
      if (submissionData.registrationType === 'DAILY') {
        ...
        submissionData.salary = submissionData.salary
          ? Number(submissionData.salary)
          : null
      } else {
        submissionData.salary = submissionData.salary
          ? Number(submissionData.salary)
      ...
      submissionData.points = submissionData.points
        ...
        : 0
      submissionData.transportAllowance = submissionData.transportAllowance
        ? Number(submissionData.transportAllowance)
        : 0
    ...
    onSuccess: async (createdOrUpdated) => {
      if (employeePhoto && createdOrUpdated?.id) {
        try {
          ...
          toast.error(
            'O funcionário foi cadastrado, mas a foto de perfil falhou.',
          )
        }
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex max-h-[90vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-[800px]">
        <DialogHeader className="border-b bg-muted/20 p-6 pb-4">
          ...
          <DialogDescription>
            {isEditing
              ? 'Atualize as informações contratuais e pessoais.'
              : 'Preencha os dados obrigatórios para cadastro.'}
          </DialogDescription>
        </DialogHeader>
                              ...
                              <>
                                ...
                                >
                                  <div className="flex items-center gap-2">
                                    <UserX className="h-4 w-4" /> Desligado /
                                    Inativo
                                  </div>
                                </SelectItem>
                              </>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          Peso para divisão da caixinha (0-10).
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  render={({ field }) => (
                    <FormItem className="flex flex-row items-center justify-between rounded-lg border bg-white p-4 shadow-sm">
                      <div className="space-y-0.5">
                        ...
                        <FormDescription>
                          O colaborador tem direito a receber cesta básica
                          mensal?
                        </FormDescription>
                      </div>
                      <FormControl>
